...
describe('Attempt API Integration', () => {
    ...
    describe('POST /save-attempt', () => {
        ...
        it('should persist multiple attempts per user', async () => {
            // Save first attempt
            const first = await request(server).post('/save-attempt').send({
                userId: 'user-456',
                language: 'python',
                code: 'def test(): pass',
                passed: false,
            ...
            // Save second attempt
            const second = await request(server).post('/save-attempt').send({
                userId: 'user-456',
                language: 'python',
            ...
            const history = await request(server).get('/users/user-456/attempts');
        ...
        it('should allow repeated attempts on same question', async () => {
            ...
            const history = await request(server).get('/users/user-789/attempts');
    ...
    describe('GET /users/:userId/attempts', () => {
        it('should return all attempts for user', async () => {
            const saveRes = await request(server).post('/save-attempt').send({
                userId: 'user-abc',
                language: 'typescript',
                code: 'const x = 1;',
                passed: true,
            ...
            const response = await request(server).get('/users/user-abc/attempts');
        ...
        it('should return newest attempts first', async () => {
            ...
            const history = await request(server).get(`/users/${userId}/attempts`);
        ...
        it('should support pagination with limit and skip', async () => {
            ...
            const page1 = await request(server).get(`/users/${userId}/attempts?limit=2`);
            ...
            const page2 = await request(server).get(`/users/${userId}/attempts?limit=2&skip=2`);
    ...
    describe('Admin and Auth routes (placeholder)', () => {
        it('should enforce user auth on read routes', () => {
            // TODO: Add test when auth middleware is implemented
            expect(true).toBe(true);
        });

        it('should enforce admin role on admin routes', () => {
            // TODO: Add test when role-based routes are implemented
            expect(true).toBe(true);
        });
    });
});
