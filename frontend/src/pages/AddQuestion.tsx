...
const AddQuestion = () => {
    ...
    useEffect(() => {
        const fetchNextQuestionId = async () => {
            try {
                ...
                const usedIds = new Set(questions.map((q: any) => q.questionId));
                let nextId = 1;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!title || !description || !categories || !difficulty || !questionType) {
        ...
        const payload: QuestionPayload = {
            ...
            examples: examples
                ...
                .filter(Boolean),
            supportedLanguages,
            executionSpec: {}, // This can be derived automatically if needed
        };
        ...
        if (questionType === 'function') {
            ...
            payload.questionType = 'function'; // Ensure questionType is set to 'function'
        }
        ...
        if (questionType === 'design') {
            payload.designSignature = {
                className,
                constructorParams: constructorParams.filter((p) => p.name.trim() && p.type.trim()),
                methods: methods
            ...
            payload.questionType = 'design'; // Ensure questionType is set to 'design'
        }
        ...
        if (questionType === 'sql') {
            ...
            payload.questionType = 'sql'; // Ensure questionType is set to 'sql'
        }
        ...
        try {
            await questionAxios.post('/questions', payload);
            navigate('/admin/questions');
        } catch (error: any) {
            console.error('Create question error:', error.response);
            setErrorMessage(
